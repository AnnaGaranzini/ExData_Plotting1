file <- "./household_power_consumption.txt"
data <- read.table(file, header=TRUE, sep=";", stringsAsFactors=FALSE, dec=".")
two_days <- data[data$Date %in% c("1/2/2007","2/2/2007") ,]

png("plot4.png", width=480, height=480)
par(mfrow = c(2, 2))
plot(d_t, two_days$Global_active_power, type="l", xlab="", ylab="Global Active Power")
plot(d_t, two_days$Voltage, type="l", xlab="datetime", ylab="Voltage")
plot(d_t, two_days$Sub_metering_1, type="l", xlab="", ylab = "Energy sub metering")
lines(d_t, two_days$Sub_metering_2, type="l", col="red")
lines(d_t, two_days$Sub_metering_3, type="l", col="blue")
legend("topright", c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col=c("black", "red", "blue"), lty=1, lwd=2, bty = "n")
plot(d_t, two_days$Global_reactive_power, type="l", xlab="datetime", ylab = "Global_reactive_power")
dev.off()